...
void helloDlg()
{
    ::MessageBox(NULL, TEXT("A stupid plugin for packets\nCreated By: Vivek Kushwaha<vivekk4@verifone.com>"), TEXT(""), MB_OK);
}

void parseXPIPacket()
{
	XPIPacketParser XPIpp;
    ...
    // Say hello now :
    // Scintilla control has no Unicode mode, so we use (char *) here
	//::SendMessage(curScintilla, SCI_SETTEXT, 0, (LPARAM)XPIpp.parsePacket("<<NUL>>").c_str());
	int end =  ::SendMessage(curScintilla, SCI_GETSELECTIONEND, 0, 0 );
	int start = ::SendMessage(curScintilla, SCI_GETSELECTIONSTART, 0, 0 );

	if ( (end - start) <= 0 ) 
	{
		::MessageBox(NULL, TEXT("lenth is less"), TEXT("Error"), MB_OK);
		return;
	}
	...
	string finalString = XPIpp.parsePacket(string(tr.lpstrText));
